...
public class NewsAction extends BaseAction {
    ...
    public String QueryNews() {
        ...
        List<News> newsList = newsDAO.QueryNewsInfo(newsType, title, publishTime, currentPage);
        /*计算总的页数和总的记录数*/
        newsDAO.CalculateTotalPageAndRecordNumber(newsType, title, publishTime);
        /*获取到总的页码数目*/
    ...
    public String QueryNewsOutputToExcel() { 
        ...
        if(title == null) title = "";
        if(publishTime == null) publishTime = "";
        List<News> newsList = newsDAO.QueryNewsInfo(newsType,title,publishTime);
        ExportExcelUtil ex = new ExportExcelUtil();
        String title = "News信息记录"; 
        String[] headers = { "新闻id","新闻分类","新闻标题","新闻主图","发布时间"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<newsList.size();i++) {
        	News news = newsList.get(i); 
        	dataset.add(new String[]{news.getNewsId() + "",news.getNewsType(),news.getTitle(),news.getNewsPhoto(),news.getPublishTime()});
        }
        /*
