...
public class VideoTypeEditActivity extends Activity {
	// 声明确定添加按钮
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// 设置当前Activity界面布局
		setContentView(R.layout.videotype_edit); 
		ImageView search = (ImageView) this.findViewById(R.id.search);
		search.setVisibility(View.GONE);
		TextView title = (TextView) this.findViewById(R.id.title);
		title.setText("编辑视频类型信息");
			...
			public void onClick(View v) {
				try {
					...
					if(ET_typeName.getText().toString().equals("")) {
						Toast.makeText(VideoTypeEditActivity.this, "类别名称输入不能为空!", Toast.LENGTH_LONG).show();
						ET_typeName.setFocusable(true);
						ET_typeName.requestFocus();
						return;	
					}
					videoType.setTypeName(ET_typeName.getText().toString());
					/*调用业务逻辑层上传视频类型信息*/
					VideoTypeEditActivity.this.setTitle("正在更新视频类型信息，稍等...");
					String result = videoTypeService.UpdateVideoType(videoType);
	...
	private void initViewData() {
		...
		this.ET_typeName.setText(videoType.getTypeName());
	}
	@Override
